...
void CustomSort::setSelectedFields(QString name, std::vector<std::string> fields){
    ui->sort_name->setText(name);
    for(int i = 0; i < fields.size(); i++){
        QComboBox* field = fieldBoxes.at(i);
        for(int j = 0; j <= field->count(); j ++){
            QString str = field->itemText(j);
            if(str.compare(QString::fromStdString(fields[i])) == 0){
                field->setCurrentIndex(j);
            }
        }
    }
}
