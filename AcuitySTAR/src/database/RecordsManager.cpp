...
/**
 * Returns the formatted QString for the Tab data. To be used with TreeModel
 *
 * @param fileType      the type of database
 * @param startYear     the start year in the range to search
 * @param endYear       the end year in the range to search
 ...
 *                      of a TreeModel
 */
QString RecordsManager::createQStringForTab(int fileType, int startYear, int endYear, const std::vector<std::string> &sortFields, char filterStart, char filterEnd)
{
    switch(fileType) {
    case PRESENTATIONS_TAB:
    case PUBLICATIONS_TAB:
        return QString(analyze(startYear, endYear, sortFields, sortFields.back(), filterStart, filterEnd).c_str()); // pres, pub
    case FUNDING_TAB:
        return QString(analyze(startYear, endYear, sortFields, list(1, "Total Amount"), 0x1, sortFields.back(), filterStart, filterEnd).c_str()); // grants
     case TEACH_TAB:
        return QString(analyze(startYear, endYear, sortFields, list(2, "Total Hours", "Number Of Trainees"), 0x0, sortFields.back(), filterStart, filterEnd).c_str()); // teaching
    }
...
/**
 * Returns the summary headers for Tab data. To be used with TreeModel.
 *
 * @param fileType  the type of database
 * @param topLevel  the header name of the top-level sort field
 * @return          a QList containing the headers for the tree view
 */
QList<QVariant> RecordsManager::createHeadersListForTab(int fileType, std::string topLevel) {
    QList<QVariant> hds;

    switch(fileType) {
    case PRESENTATIONS_TAB:
        hds << topLevel.c_str() << "# Presentations"; // pres
        break;
    case PUBLICATIONS_TAB:
        hds << topLevel.c_str() << "Totals"; // pubs
        break;
    case FUNDING_TAB:
        hds << topLevel.c_str() << "Total" << "Total $"; // grants
        break;
    case TEACH_TAB:
        hds << topLevel.c_str() << "Totals" << "# of Hours" << "# of Students"; // teaching
        break;
    }
 ...
 */
std::vector<std::pair<std::string, double>> RecordsManager::getCountTuple(int startYear, int endYear, const std::vector<std::string> &headers, const std::vector<std::string> &matchStrings, char filterStart, char filterEnd) {
...
std::string RecordsManager::analyze(StringTree sortedTree, const std::vector<int> &sortFields, const std::vector<int> &accs, std::vector<double> &accsTotals,
    ...
    for (StringTree::iterator sortField = sortedTree.begin(); sortField != sortedTree.end(); sortField = uniqueValue.second) {
        ...
        for (StringTree::iterator itr = uniqueValue.first; itr != uniqueValue.second; itr++) {
            ...
            if ((int) sortFields.size() > depth + 1) {
